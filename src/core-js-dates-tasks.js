...
function getTime(date) {
  ...
  function typeOfDate(func) {
    const value = dateObj[func]();
    return value < 10 ? `0${value}` : value;
  }
  return `${typeOfDate('getHours')}:${typeOfDate('getMinutes')}:${typeOfDate('getSeconds')}`;
}
...
function getNextFriday(date) {
  let toFriday;
  if (date.getDay() >= 5) {
    toFriday = 5 - date.getDay() + 7;
  } else toFriday = 5 - date.getDay();

  const nextFriday = new Date(date);
  nextFriday.setDate(nextFriday.getDate() + toFriday);
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return Math.ceil((dataEnd - dataStart) / (24 * 60 * 60 * 1000)) + 1;
}
...
function isDateInPeriod(date, period) {
  ...
  return givenDate >= startPeriod && givenDate <= endPeriod;
}
...
function formatDate(date) {
  ...
  const resultDate = `${month}/${day}/${year}, ${hours12}:${formatMinutes}:${formatSeconds} ${a}`;
...
function getWeekNumberByDate(date) {
  ...
  const day =
    Math.ceil((thursday - firstDateOfYear) / (24 * 60 * 60 * 1000)) + 1;
...
function getNextFridayThe13th(date) {
  ...
  for (let i = newData.getMonth(); ; i += 1) {
    const conditionDate = new Date(newData.getFullYear(), i - 1, 13);
    if (conditionDate.getDay() === 5) return conditionDate;
  }
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const startOfPeriod = new Date(period.start.split('-').reverse().join('-'));
  const endOfPeriod = new Date(period.end.split('-').reverse().join('-'));
  const schedule = [];
  const currentDate = new Date(startOfPeriod);
  while (currentDate <= endOfPeriod) {
    for (let i = 0; i < countWorkDays && currentDate <= endOfPeriod; i += 1) {
      const day = currentDate.getDate();
      const month = currentDate.getMonth() + 1;
      schedule.push(
        `${day < 10 ? `0${day}` : day}-${month < 10 ? `0${month}` : month}-${currentDate.getFullYear()}`
      );
